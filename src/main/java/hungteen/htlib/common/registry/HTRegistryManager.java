 ...
 * 客户端同步。所有 {@link HTCodecRegistry} 都会登记在
 * {@link #CODEC_REGISTRIES}（名称 → 注册表 的双向表）中。</p>
 *
...
public class HTRegistryManager {
     ...
     * 以及玩家加入服务器时触发），由 {@link HTLib#HTLib()} 挂接：
     * 无玩家（event.getPlayer() == null，全服广播）时逐人同步，否则只同步给针对于该玩家。</p>
     */
     ...
     * 在 Java 代码中注册，随 Forge 的 mod 加载事件真正填充到注册表中，
     * 之后可直接通过 {@code byNameCodec()} 序列化为 ResourceLocation。</p>
     *
     ...
     * （自带名字/命名空间/注册名），可直接用对象本身注册，专为轻量"类型/选项"
     * 类对象设计，方便后续用一个循环去注册其它常规注册。</p>
     *
     * <p><b>注意：</b>不要为同一个注册名创建多个注册表。</p>
     */
     ...
     * 创建一个"后于原版注册"的数据包注册表（默认不进行同步）。
     *
     * @param requireCache 服务端是否需要缓存已加载条目的 key 列表（{@link hungteen.htlib.api.interfaces.IHTCodecRegistry#getCachedKeys}）。
     */
     ...
     *
     * @param clazz 数据类（T 的具体运行时类型），有 clazz 才说明采用 HTLib 方式同步而非原版同步。
     */
     ...
     * <p><b>注意：</b>不要为同一个注册名创建多个注册表。</p>
     *
     * @param syncSup 用于数据包的同步 codec；为 null 表示不同步。
     * @param clazz 数据类；有 clazz 表示采用 HTLib 的自定义方式同步数据，否则采用原版同步方法。
     * @param requireCache 是否需要缓存数据（服务端），供客户端等场景查询已加载条目的 key 列表。
     */
