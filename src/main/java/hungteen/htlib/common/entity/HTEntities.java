...
public class HTEntities{
    ...
    public static final RegistryObject<EntityType<HTBoat>> CHEST_BOAT = ENTITY_TYPES.register("chest_boat", () -> {
        return EntityType.Builder.<HTBoat>of(HTChestBoat::new, MobCategory.MISC).sized(1.375F, 0.5625F).clientTrackingRange(10).build(HTLibHelper.prefix("chest_boat").toString());
    });

    public static final RegistryObject<EntityType<SeatEntity>> SEAT = ENTITY_TYPES.register("seat", () -> {
        return EntityType.Builder.<SeatEntity>of(SeatEntity::new, MobCategory.MISC).sized(0F, 0F).clientTrackingRange(10).build(HTLibHelper.prefix("chest_boat").toString());
    });
